...
import time
import zlib
from gitty.utils import delete_file_and_parent_folders, get_current_head_hash, read_from_blob, read_objects, write_to_file
# sys.path.append(str(Path(__file__).parent.resolve()))

class _GitObjectsMeta(type):
    @property
    def repo_path(cls) -> Path:
        return Path.cwd().resolve()

    @property
    def index_file_path(cls) -> Path:
        return Path.cwd().resolve() / '.gitty' / 'index.json'

    @property
    def objects_path(cls) -> Path:
        return Path.cwd().resolve() / '.gitty' / 'objects'

class Git_objects(metaclass=_GitObjectsMeta):    
    ...
    def handle_save(self,hash,compressed_content):
        ...
        blob_folder_path=Git_objects.objects_path/folder_name
    ...
    @classmethod
    def save_from_remote_compressed(cls, obj_hash: str, raw_compressed_bytes: bytes):
        """Saves already-compressed bytes directly into .gitty/objects/."""
        folder_name, file_name = obj_hash[:2], obj_hash[2:]
        blob_folder_path = Path.cwd() / '.gitty' / 'objects' / folder_name
        blob_folder_path.mkdir(parents=True, exist_ok=True)
        
        with open(blob_folder_path / file_name, 'wb') as f:
            f.write(raw_compressed_bytes)
...
class Commit(Git_objects):
    ...
    def commit_content(commit_hash):
        ...
        content_blob = Commit.read_commit(commit_hash)
        # Adjust this index [1] if your read_from_blob returns (header, body)
        new_tree_hash = content_blob["tree"]
    ...
    @staticmethod
    def DFS(commit_hash,target_hash=None):
        seen_commit = set()
        ...
        while stack:
            ...
            if target_hash and current_commit == target_hash:
                return True
            
            seen_commit.add(current_commit)            
            if "parent" in commit_content:
                for parent_hash in reversed(commit_content['parent']):
                    stack.append(parent_hash)

        return False if target_hash else True


# Commit.DFS("17c5390177551d25ca04ea600bd9200dd6c4ab6e","50f43cdbb8bdfb9a15d9709df415597975956757")