...
from gitty.utils import get_current_branch, get_file_from_commit, get_current_head_hash, read_from_blob, temp_index_exist

# TODO: check if the gitty is initalized first
...
class Index:
    ...
    def check_conflict_in_file(blob,file_path):
        ...
        while i<len(file_contents):
            if file_contents[i:i+window_size] == MARKER_HEAD+MARKER_BASE+MARKER_THEIRS:
                print(bcolors.RED)
    ...
    def status(self):
        ...
        print("\n")
        print(bcolors.BOLD + "changes to be staged" + bcolors.ENDC)
            ...
            if not any(part.startswith('.') for part in file.parts):
                if file.is_file():
                    ...
                    index[rel_key] = {'hash':file_hash,
                                        'ct_time':time.ctime(stats.st_birthtime ),
