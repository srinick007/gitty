...
def get_config_value(repo_path, section, key, default="Unknown"):
    config = configparser.ConfigParser()
    
    # List of paths to check: Local first, then Global
    config_paths = [
        repo_path / '.gitty' / 'config',
        Path.home() / '.gittyconfig'
    ]
...
def set_config_value(key, value, section, repo_path=None, is_global=False):
    if repo_path is None:
        repo_path = Path.cwd().resolve()
        
    config = configparser.ConfigParser()
    path = Path.home() / '.gittyconfig' if is_global else repo_path / '.gitty' / 'config'
...
def set_upstream(branch_name: str, remote_name: str = "origin", remote_branch: str = None, repo_path: Path = None):
    """Writes branch.<name>.remote and branch.<name>.merge to .gitty/config"""
    target_branch = branch_name.strip()
    r_branch = (remote_branch if remote_branch else target_branch).strip()
    
    set_config_value(
        key='remote',
        value=remote_name.strip(),
        section=f'branch "{target_branch}"',
        repo_path=repo_path
    )
    set_config_value(
        key='merge',
        value=f'refs/heads/{r_branch}',
        section=f'branch "{target_branch}"',
        repo_path=repo_path
    )

def get_upstream(branch_name: str = None, repo_path: Path = None) -> tuple[str | None, str | None]:
    """Returns (remote_name, remote_branch_name) for a given local branch."""
    target_branch = (branch_name if branch_name else get_current_branch(repo_path)).strip()
    
    remote = get_config_value(repo_path, f'branch "{target_branch}"', 'remote', default=None)
    merge_ref = get_config_value(repo_path, f'branch "{target_branch}"', 'merge', default=None)
    
    remote_branch = None
    if merge_ref:
        remote_branch = merge_ref.replace("refs/heads/", "").strip()
        
    return remote, remote_branch

def get_local_refs(repo_path):
    if repo_path:
        refs_path = repo_path / '.gitty' / 'refs' / 'heads'
    else:
        refs_path = get_repo_path() / '.gitty' / 'refs' / 'heads'
    if not refs_path.exists():
        return None

    refs = {}
    for ref in refs_path.rglob("*"):
...
def get_current_head_hash():
    head_path = get_repo_path() / '.gitty' / 'HEAD'
    ...
    if content.startswith('ref:'):
        ref_path = get_repo_path() / '.gitty' / content.split(' ')[1]
        if ref_path.exists():
...
def get_branch_hash(branch):
    branch_path = get_repo_path() / '.gitty' / 'refs' / 'heads' / branch
...
def temp_index_exist():
        path = get_repo_path() / '.gitty' / 'temp_merge_index.json'
        return path.exists()
...
def delete_file_and_parent_folders(file):
    ...
    if file.exists():
        ...
        for parent in file.parents:
            if parent == get_repo_path() or parent == Path('.'):
                break
...
def handle_save(hash,compressed_content):
    ...
    blob_folder_path=get_blob_path()/folder_name
...
def read_from_blob(blob):
    ...
    with (get_blob_path()/folder_name/file_name).open('rb') as file:
        content = file.read()
...
# print(read_from_blob("fafd0a0f0b037265d16f89f90c16f5e4562de4fe"))
...
def read_objects(root_tree,path,level = 0,):
    ...
    for key,value in tree_content.items():
        if value[0] == "Tree":
            ...
            if path:
                file_path = get_repo_path() / f'{'/'.join(path)}/{value[1]}'
            else:
                file_path = get_repo_path() / value[1]
            rel_path = file_path.relative_to(get_repo_path()).as_posix()
            # print(key)
...
def get_file_from_commit(root_tree,file,path_list=[]):
    ...
    for key,value in tree_content.items():
        if value[0] == "Tree":
            ...
            if path_list:
                file_path = get_repo_path() / f'{'/'.join(path_list)}/{value[1]}'
            else:
                file_path = get_repo_path() / value[1]
            rel_path = file_path.relative_to(get_repo_path()).as_posix()
            if rel_path == file:
...
def gitty_helper(A_content, B_content, conflict_level, merge_branch,dont_run=False):
    ...
    MARKER_BASE = "=" * 7
    MARKER_THEIRS = ">" * 7
    temp.append(MARKER_HEAD+MARKER_BASE+MARKER_THEIRS)
...
def modify_delete_conflict_helper(branch_name, blob_hash):
    ...
    if user_decision == 'later':
        ...
        conflict_content = [
            MARKER_HEAD+MARKER_BASE+MARKER_THEIRS + " (Deleted)",
